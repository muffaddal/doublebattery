...
if ( $shop_page_id && $shop_page && strstr( $permalinks['product_base'], '/' . $shop_page->post_name ) && get_option( 'page_on_front' ) != $shop_page_id ) {
	$prepend = '<a href="' . get_permalink( $shop_page ) . '">' . $shop_page->post_title . '</a> ' . $delimiter;
}
...
if ( ( ! is_front_page() && ! ( is_post_type_archive() && get_option( 'page_on_front' ) == wc_get_page_id( 'shop' ) ) ) || is_paged() ) {
	...
	if ( ! empty( $home ) ) {
		echo '<a class="home" href="' . apply_filters( 'woocommerce_breadcrumb_home_url', esc_url( home_url('/') ) ) . '">' . $home . '</a>' . $delimiter;
	}

	if ( is_home() ) {

		echo wp_kses_post($before . single_post_title('', false) . $after);
		...
		if ( 0 != $this_category->parent ) {
			...
			if ( ( $parents = get_category_parents( $parent_category, TRUE, $after . $delimiter . $before ) ) && ! is_wp_error( $parents ) ) {
				echo wp_kses_post($before . rtrim( $parents, $after . $delimiter . $before ) . $after . $delimiter);
			}
		}
		...
		echo wp_kses_post($before . esc_html( $current_term->name ) . $after);

	} elseif ( is_tax( 'product_tag' ) ) {

		$queried_object = $wp_query->get_queried_object();
		echo wp_kses_post($prepend . $before . esc_html_e( 'Products tagged &ldquo;', 'woocommerce' ) . $queried_object->name . '&rdquo;' . $after);
		...
		if ( is_search() ) {
			...
			echo '<a href="' . get_post_type_archive_link( 'product' ) . '">' . $_name . '</a>' . $delimiter . esc_html_e( 'Search results for &ldquo;', 'woocommerce' ) . get_search_query() . '&rdquo;';

		} elseif ( is_paged() ) {

			echo '<a href="' . get_post_type_archive_link( 'product' ) . '">' . $_name . '</a>';
		...
		if ( 'product' == get_post_type() ) {
			...
			if ( $terms = wc_get_product_terms( $post->ID, 'product_cat', array( 'orderby' => 'parent', 'order' => 'DESC' ) ) ) {
				...
				echo '<a href="' . get_term_link( $main_term ) . '">' . $main_term->name . '</a>' . $delimiter;
		...
		echo wp_kses_post($before . esc_html_e( 'Error 404', 'woocommerce' ) . $after);
		...
		foreach ( $breadcrumbs as $crumb ) {
			echo wp_kses_post($before . $crumb . $after . $delimiter);
		}
		...
		echo wp_kses_post($before . esc_html_e( 'Search results for &ldquo;', 'woocommerce' ) . get_search_query() . '&rdquo;' . $after);

	} elseif ( is_tag() ) {

			echo wp_kses_post($before . esc_html_e( 'Posts tagged &ldquo;', 'woocommerce' ) . single_tag_title( '', false ) . '&rdquo;' . $after);

	} elseif ( is_author() ) {

		$userdata = get_userdata( $author );
		echo wp_kses_post($before . esc_html_e( 'Author:', 'woocommerce' ) . ' ' . $userdata->display_name . $after);

	}

	if ( get_query_var( 'paged' ) ) {
		echo ' (' . esc_html_e( 'Page', 'woocommerce' ) . ' ' . get_query_var( 'paged' ) . ')';
	}
