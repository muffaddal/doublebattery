...
class cmb_Meta_Box_types {
	...
	public function render_repeatable_field() {
		...
		<p class="add-row">
			<a data-selector="<?php echo esc_attr($table_id); ?>" class="add-row-button button" href="#"><?php esc_html_e( 'Add Row', 'cmb' ); ?></a>
		</p>
	...
	protected function repeat_row( $class = 'repeat-row' ) {
		...
		<tr class="<?php echo esc_attr($class); ?>">
			...
			<td class="remove-row">
				<a class="button remove-row-button" href="#"><?php esc_html_e( 'Remove', 'cmb' ); ?></a>
			</td>
		</tr>
		...
		if ( $meta_value && is_array( $meta_value ) ) {
			...
			foreach ( $meta_value as $id => $fullurl ) {
				...
				if ( $this->is_valid_img_ext( $fullurl ) ) {
					...
					echo
					'<li>',
						__( 'File:', 'cmb' ), ' <strong>', $title, '</strong>&nbsp;&nbsp;&nbsp; (<a href="', esc_url($fullurl), '" target="_blank" rel="external">'. esc_html__( 'Download', 'cmb' ) .'</a> / <a href="#" class="cmb_remove_file_button">'. esc_html__( 'Remove', 'cmb' ) .'</a>)
						'. $id_input .'
					</li>';
				}
			}
		...
		'<div id="', $this->_id( '_status' ) ,'" class="cmb_media_status">';
			if ( ! empty( $meta_value ) ) {
				...
				if ( $this->is_valid_img_ext( $meta_value ) ) {
					...
					echo __( 'File:', 'cmb' ), ' <strong>', $title, '</strong>&nbsp;&nbsp;&nbsp; (<a href="', esc_url($meta_value), '" target="_blank" rel="external">'. esc_html__( 'Download', 'cmb' ) .'</a> / <a href="#" class="cmb_remove_file_button" rel="', $cached_id, '">'. esc_html__( 'Remove', 'cmb' ) .'</a>)';
				}
			}
		echo '</div>';
	}
