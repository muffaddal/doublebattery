...
class cmb_Meta_Box {
	...
	public static function autoload_helpers( $class_name ) {
		...
		$dir = str_replace('/',DIRECTORY_SEPARATOR, get_template_directory() . '/lib/metabox');

		$file = "$dir/helpers/$class_name.php";
		if ( file_exists( $file ) )
			//include( $file );
			get_template_part( 'lib/metabox/helpers/'.$class_name );
	}
			...
			if ( ! is_admin() ) {
		   	...
		   	wp_register_script( 'wp-color-picker', admin_url( 'js/color-picker.min.js' ), array( 'iris' ), self::CMB_VERSION );
				wp_localize_script( 'wp-color-picker', 'wpColorPickerL10n', array(
					'clear'         => esc_html__( 'Clear' , 'fatotheme' ),
					'defaultString' => esc_html__( 'Default' , 'fatotheme'),
					'pick'          => esc_html__( 'Select Color' , 'fatotheme'),
					'current'       => esc_html__( 'Current Color', 'fatotheme' ),
				) );
			}
		} else {
	...
	public static function get_meta_box_url() {
		$dir_file = str_replace('/',DIRECTORY_SEPARATOR, get_template_directory() . '/lib/metabox');
		if ( strtoupper( substr( PHP_OS, 0, 3 ) ) === 'WIN' ) {
			// Windows
			$content_dir = str_replace( '/', DIRECTORY_SEPARATOR, WP_CONTENT_DIR );
			//$content_url = str_replace( $content_dir, WP_CONTENT_URL, get_template_directory_uri() . '/lib/metabox/' );
			$content_url = str_replace( $content_dir, WP_CONTENT_URL, $dir_file );
		...
		return apply_filters('cmb_meta_box_url', $cmb_url );
	}
...
function cmb_metabox_form( $meta_box, $object_id, $echo = true ) {
	...
	$form = sprintf( $form_format, $meta_box['id'], $object_id, $form, esc_html__( 'Save' ,'fatotheme' ) );
