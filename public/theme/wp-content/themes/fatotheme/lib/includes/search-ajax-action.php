<?php
function fatotheme_result_search_callback()
{
	ob_start();
	...
	if ($keyword) {
		...
		if ($search && count($search->post) > 0) {
			...
			$newdata[] = array(
				'id' => -1,
				'title' => esc_html__('Sorry, but nothing matched your search terms. Please try again with different keywords.', 'fatotheme'),
				'guid' => '',
				'date' => null,
				'shortdesc' => ''
			);
...
add_action('wp_ajax_nopriv_result_search', 'fatotheme_result_search_callback');
add_action('wp_ajax_result_search', 'fatotheme_result_search_callback');