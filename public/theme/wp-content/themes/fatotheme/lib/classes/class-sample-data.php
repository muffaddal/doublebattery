 ...
 */
class LaneThemes_SampleData extends LaneThemes_Base
{
	...
	private function init_filesystem_api( $url = '', $action = -1, $name = '_wpnonce' ) {
		if ( ! $url ) {
	...
	public function admin_menu() {
		add_theme_page( esc_html__( 'Sample Data Installation', 'fatotheme' ),
						esc_html__( 'Install Sample Data', 'fatotheme' ),
	...
	public function admin_page() {
		?>
			...
			<div id="sample-data-installer">
				<h1><?php esc_html_e( 'Sample Data Installation', 'fatotheme' ) ?></h1>

				<div class="start-screen">
					<p><?php esc_html_e( 'There is following tasks will be run for install sample data:', 'fatotheme' ) ?></p>
					<ol class="tasks">
						<li><?php esc_html_e( 'Import sample content', 'fatotheme' ) ?></li>
						<li><?php esc_html_e( 'Download media files', 'fatotheme' ) ?></li>
					</ol>

					<p>
						<button type="button" id="install-sample-data" class="button-primary"><?php esc_html_e( 'Install Sample Data', 'fatotheme' ) ?></button>
					</p>
				</div>
				...
				<div class="running-screen">
					<ol class="tasks">
						<li data-task="import-content">
							<span><?php esc_html_e( 'Import sample content', 'fatotheme' ) ?></span>
							<span class="spinner-text"></span>
						</li>
						<li data-task="download-attachments">
							<span><?php esc_html_e( 'Download media files', 'fatotheme' ) ?></span>
					...
					<p class="finish-actions">
						<span><?php esc_html_e( 'Congratulation! Sample data has been installed successfully', 'fatotheme' ) ?></span>
					</p>
				</div>
			</div>
	...
	public function enqueue( $page ) {
		if ( $page == 'appearance_page_sample-data' ) {
			...
			wp_localize_script( 'theme-sample-data', '_sampleDataLocalization', array(
				'confirm_installation' => esc_html__( 'Attention!!! Your existing data will be removed when install sample data. Are you sure you want to install sample data?', 'fatotheme' )
			) );
	...
	public function import_content( $response, $context ) {
		...
		if ( $context['step'] == 'import-content' && wp_verify_nonce( $context['nonce'], 'sample_data_installation' ) ) {
			...
			if ( ! is_file( $sample_data_path ) )
				throw new Exception( sprintf( esc_html__( 'Sample data file not found: %s', 'fatotheme' ), $sample_data_path ) );

			if ( ! $this->init_filesystem_api() )
				throw new Exception( esc_html__( 'Cannot request permision for FileSystem API', 'fatotheme' ) );
			...
			if ( isset( $post_types['testimonial'] ) )
				array_push( $allowed_groups, 'testimonial' );

			if ( isset( $post_types['ourteam'] ) )
				array_push( $allowed_groups, 'ourteam' );

			if ( isset( $post_types['footer'] ) )
				array_push( $allowed_groups, 'footer' );
		...
		if ( $context['step'] == 'download-attachment' && wp_verify_nonce( $context['nonce'], 'sample_data_download_attachments' ) ) {
			if ( isset( $context['id'] ) ) {
				...
				$remote_response = wp_safe_remote_get( $attachment->guid, array(
					'timeout' => 9999, 'stream' => true, 'filename' => $destination ) );
