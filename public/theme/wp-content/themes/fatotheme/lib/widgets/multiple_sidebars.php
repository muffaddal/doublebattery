...
class sidebar_generator {
		...
		if ( is_array( $sidebars ) ) {
			foreach ( $sidebars as $sidebar ) {
				...
				register_sidebar( array(
					'name'          => $sidebar,
					'id'            => 'fatotheme-custom-sidebar-' . strtolower( $sidebar_class ),
	...
	public static function add_meta_boxes() {
		...
		foreach ( $post_types as $post_type ) {
			add_meta_box( 'sbg_box', esc_html__( 'Sidebar', 'fatotheme' ), array( 'sidebar_generator', 'edit_form' ), $post_type, 'side' );
		}
	}
	...
	public static function add_sidebar(){
		...
		if ( isset( $sidebars[ $id ] ) ) {
			die("alert('" . esc_html__( 'Widget Section already exists, please use a different name.', 'fatotheme' ) . "')" );
		}
	...
	public static function remove_sidebar(){
		...
		$no_widget_text = esc_html__( 'No Widget Sections defined.', 'fatotheme' );

		$id = sidebar_generator::name_to_class( $name );
		if ( ! isset( $sidebars[ $id ] ) ) {
			die( 'alert("' . esc_html__( 'Sidebar does not exist.', 'fatotheme' ) . '")' );
		}
	...
	public static function admin_menu() {
		add_theme_page( esc_html__( 'Widget Sections', 'fatotheme' ), esc_html__( 'Widget Sections', 'fatotheme' ), 'manage_options', 'multiple_sidebars', array( 'sidebar_generator', 'admin_page' ) );
	}

	public static function admin_page() { ?>
		<script>
		function remove_sidebar_link(name,num){
			answer = confirm( "<?php esc_html_e( 'Are you sure you want to remove', 'fatotheme' ); ?> " + name + "?\n<?php esc_html_e( 'This will remove any widgets you have assigned to this widget section.', 'fatotheme' ); ?>" );
				if ( answer ) {
					//alert('AJAX REMOVE');
			...
			function add_sidebar_link(){
				var sidebar_name = prompt( "<?php esc_html_e( 'Widget Section Name:', 'fatotheme' ); ?>","" );
				//alert(sidebar_name);
		...
		<div class="postbox" style="max-width:1719px;">
			<h2 class="hndle ui-sortable-handle" style="padding: 15px 12px; margin: 0;">
				<span><?php esc_html_e( 'Widget Sections', 'fatotheme' ); ?></span>
			</h2>
			<div class="inside" style="margin-bottom: 0;">
				<table class="widefat page" id="sbg_table">
					<tr>
						<th><?php esc_html_e( 'Widget Section Name', 'fatotheme' ); ?></th>
						<th><?php esc_html_e( 'CSS Class', 'fatotheme' ); ?></th>
						<th><?php esc_html_e( 'Remove', 'fatotheme' ); ?></th>
					</tr>
					<?php
					$sidebars = sidebar_generator::get_sidebars();
					?>
					<?php if ( is_array( $sidebars ) && ! empty( $sidebars ) ) : ?>
						...
						<?php foreach ( $sidebars as $sidebar ) : ?>
							...
							<tr class="<?php echo esc_attr($alt); ?>">
								<td><?php echo wp_kses_post($sidebar); ?></td>
								<td><?php echo sidebar_generator::name_to_class( $sidebar ); ?></td>
								<td><a href="javascript:void(0);" onclick="return remove_sidebar_link('<?php echo wp_kses_post($sidebar); ?>',<?php echo esc_html($cnt) + 1; ?>);" title="<?php esc_html_e( 'Remove this Widget Section', 'fatotheme' ); ?>"><?php esc_attr_e( 'remove', 'fatotheme' ); ?></a></td>
							</tr>
							<?php $cnt++; ?>
						<?php endforeach; ?>
					<?php else : ?>
						<tr id="no-widget-sections">
							<td colspan="3"><?php esc_html_e( 'No Widget Sections defined.', 'fatotheme' ); ?></td>
						</tr>
					<?php endif; ?>
				</table>
				<p class="add_sidebar"><a href="javascript:void(0);" onclick="return add_sidebar_link()" title="<?php _e( 'Add New Widget Section', 'fatotheme' ); ?>" class="button button-primary"><?php _e( 'Add New Widget Section', 'fatotheme' ); ?></a></p>
			</div>
		</div>
		<?php
	}
	...
	*/
	...
	public static function edit_form() {
		...
		<div class="pyre_metabox_field">
			...
			<div class="pyre_desc">
				<label><?php esc_html_e( 'Select Sidebar 1:', 'fatotheme' ); ?></label>
				<p><?php esc_html_e( 'Select sidebar 1 that will display on this page. Choose "No Sidebar" for full width.', 'fatotheme' ); ?></p>
			</div>
			<div class="pyre_field">
				...
				<?php for ( $i = 0; $i < 1; $i++ ) : ?>
					...
					<select name="sidebar_generator[<?php echo esc_attr($i); ?>]" style="display: none;">
						<option value="0"<?php echo ( '' == $selected_sidebar[ $i ] ) ? ' selected' : ''; ?>><?php esc_html__( 'WP Default Sidebar', 'fatotheme' ); ?></option>
						<?php $sidebars = $wp_registered_sidebars; // sidebar_generator::get_sidebars(); ?>
						<?php if ( is_array( $sidebars ) && ! empty( $sidebars ) ) : ?>
							<?php foreach ( $sidebars as $sidebar ) : ?>
								<?php if ( $selected_sidebar[ $i ] == $sidebar['name'] ) : ?>
									<?php if ( 'Blog Sidebar' == $sidebar['name'] || esc_html__( 'Blog Sidebar', 'fatotheme' ) == $sidebar['name'] ) : ?>
										<option value="<?php echo esc_attr( $sidebar['name'] ); ?>" selected><?php esc_html_e( 'Default Sidebar', 'fatotheme' ); ?></option>
									<?php else : ?>
										<option value="<?php echo esc_attr( $sidebar['name'] ); ?>" selected><?php esc_html( $sidebar['name'] ); ?></option>
									<?php endif; ?>
								<?php else : ?>
									<?php if ( 'Blog Sidebar' == $sidebar['name'] || esc_html__( 'Blog Sidebar', 'fatotheme' ) == $sidebar['name'] ) : ?>
										<option value="<?php echo esc_attr( $sidebar['name'] ); ?>"><?php esc_html_e( 'Default Sidebar', 'fatotheme' ); ?></option>
									<?php else : ?>
					...
					<select name="sidebar_generator_replacement[<?php echo esc_attr($i); ?>]">
						<option value="" <?php echo ( '' == $selected_sidebar_replacement[ $i ] && 'post' != $screen->post_type ) ? ' selected' : ''; ?>><?php esc_html_e( 'No Sidebar', 'fatotheme' ); ?></option>
						...
						<?php if ( is_array( $sidebar_replacements ) && ! empty( $sidebar_replacements ) ) : ?>
							<?php foreach ( $sidebar_replacements as $sidebar ) : ?>
								<?php if ( '0' == $selected_sidebar_replacement[ $i ] ) : ?>
									<?php $selected_sidebar_replacement[ $i ] = esc_html__( 'Blog Sidebar', 'fatotheme' ); ?>
								<?php endif; ?>
								...
								<?php else : ?>
									<?php if ( 'Blog Sidebar' == $sidebar['name'] || esc_html__( 'Blog Sidebar', 'fatotheme' ) == $sidebar['name'] ) : ?>
										...
										<option value="<?php echo esc_attr( $sidebar['name'] ); ?>"<?php echo wp_kses_post($selected); ?>><?php esc_html_e( 'Default Sidebar', 'fatotheme' ); ?></option>
									<?php else : ?>
										<option value="<?php echo esc_attr( $sidebar['name'] ); ?>"/><?php esc_html( $sidebar['name'] ); ?></option>
									<?php endif; ?>
								<?php endif; ?>
		...
		<div class="pyre_metabox_field">
			<input name="sbg_edit" type="hidden" value="sbg_edit" />
			<div class="pyre_desc">
				<label><?php esc_html_e( 'Select Sidebar 2:', 'fatotheme' ); ?></label>
				<p><?php esc_html_e( 'Select sidebar 2 that will display on this page. Sidebar 2 can only be used if sidebar 1 is selected.', 'fatotheme' ); ?></p>
			</div>
			<div class="pyre_field">
				<?php global $wp_registered_sidebars; //var_dump($wp_registered_sidebars); ?>
				<?php for ( $i = 0; $i < 1; $i++ ) : ?>
					<div class="fusion-shortcodes-arrow">&#xf107;</div>
					<select name="sidebar_2_generator[<?php echo esc_attr($i); ?>]" style="display: none;">
						<option value="0"<?php echo ( '' == $selected_sidebar_2[ $i ] ) ? ' selected' : ''; ?>><?php esc_html_e( 'WP Default Sidebar', 'fatotheme' ); ?></option>
						<?php $sidebars = $wp_registered_sidebars; // sidebar_generator::get_sidebars(); ?>
						<?php if ( is_array( $sidebars ) && ! empty( $sidebars ) ) : ?>
							<?php foreach ( $sidebars as $sidebar ) : ?>
					...
					<select name="sidebar_2_generator_replacement[<?php echo esc_attr($i); ?>]">
						<option value=""<?php echo ( '' == $selected_sidebar_replacement[ $i ] ) ? ' selected' : ''; ?>><?php esc_html_e( 'No Sidebar', 'fatotheme' ); ?></option>
						<?php $sidebar_replacements = $wp_registered_sidebars; //sidebar_generator::get_sidebars(); ?>
						<?php if ( is_array( $sidebar_replacements ) && ! empty( $sidebar_replacements ) ) : ?>
							<?php foreach ( $sidebar_replacements as $sidebar ) : ?>
	...
	public static function get_sidebar( $name = '0' ){
		if ( ! is_singular() ) {
			$sidebar = ( '0' != $name ) ? $name : 'fatotheme-blog-sidebar';
			...
			if ( is_array( $selected_sidebar ) && ! empty( $selected_sidebar ) ) {
				...
				for ( $i = 0; $i < $sizeof_selected_sidebar; $i++ ) {
					if ( '0' == $name && '0' == $selected_sidebar[ $i ] && '0' == $selected_sidebar_replacement[ $i ] ) {
						dynamic_sidebar( 'fatotheme-blog-sidebar' ); //default behavior
						$did_sidebar = true;
						break;
					} elseif ( '0' == $name && '0' == $selected_sidebar[ $i ] || 'Blog Sidebar' == $selected_sidebar[ $i ] || esc_html__( 'Blog Sidebar', 'fatotheme' ) == $selected_sidebar[ $i ] ) {
